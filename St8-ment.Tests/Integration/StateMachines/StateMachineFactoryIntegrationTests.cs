...
public class StateMachineFactoryIntegrationTests
{
    ...
    public class InitialStateConfiguration : IExtendedStateConfiguration
    {
        public StateId StateId => TestStateId.New;

        public void Configure(IExtendedStateComponentBuilder builder)
        {
            builder
    ...
    public class ProcessingStateConfiguration : IExtendedStateConfiguration
    {
    ...
    {
        // Arrange
        var factory = this.provider.GetRequiredService<IStateMachineProvider<string>>();
        var stateMacine = factory.Get("TEST1");
        ...
        Assert.Equal(TestStateId.Complete.Value, stateMacine.Current.Value);
    }
    ...
    public async Task ShouldVerifyPathPathOnStateMachineTest2()
    {
        // Arrange
        var factory = this.provider.GetRequiredService<IStateMachineProvider<string>>();
        var stateMacine = factory.Get("TEST2");
    ...
    public async Task ShouldVerifyDefaultPathOnStateMachineTest2()
    {
        ...
        Assert.Equal(TestStateId.Fault.Value, stateMacine.Current.Value);
    }
}
