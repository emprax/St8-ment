...
public class StateMachineFactoryTests
{
    ...
    private readonly IStateMachineProvider<string> factory;

    public StateMachineFactoryTests()
    {
        this.core = Mock.Of<IStateMachineCore>(MockBehavior.Strict);
        this.factory = new StateMachineProvider<string>(new ConcurrentDictionary<string, Func<IStateMachineCore>>(new[]
        {
    ...
    {
        // Act & Assert
        Assert.Null(this.factory.Get("NONE"));
    }
    ...
    public void ShouldCreateStateMachine()
    {
        ...
        // Act
        var result = this.factory.Get("TEST");
