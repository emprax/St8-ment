...
public class StateComponentCollection : IKeyValueStateComponent<StateId>
{
    ...
    public StateComponentCollection() => this.States = new ConcurrentDictionary<StateId, IStateComponent>();

    public bool TryGetValue(StateId key, out IStateComponent? component) => this.States.TryGetValue(key, out component);

    public void Add(StateId key, IStateComponent component) => this.States.Add(key, component);

    public Task<StateTransitionResponse> Apply<TInput>(TInput input, StateId id)
    {
        var defaultResponse = StateMachineResponse.ToUnknownState(id.Value);
