...
public class StateComponent : IKeyValueStateComponent<string>
{
    ...
    public StateComponent() => this.Components = new ConcurrentDictionary<string, IStateComponent>();

    public void Add(string key, IStateComponent component) => this.Components.Add(key, component);

    public bool TryGetValue(string key, out IStateComponent component) => this.Components.TryGetValue(key, out component);
    ...
    public async Task<StateTransitionResponse> Apply<TInput>(TInput input, StateId id)
    {
        ...
        if (this.Components.TryGetValue(typeof(object).FullName!, out var defaultComponent) && defaultComponent != null)
        {
            ...
            return response?.Response?.Succeeded ?? false
                ? new StateTransitionResponse(StateMachineResponse.ToDefaultTransition(response.State.Value, typeof(TInput).Name!), response.State)
                : response!;
        }

        return new(StateMachineResponse.ToUnspecified(id.Value, typeof(TInput).Name), id);
    }
}
