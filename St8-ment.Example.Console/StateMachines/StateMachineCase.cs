...
public class StateMachineCase
{
    ...
    private static async Task HappyFlow(IServiceProvider provider)
    {
        ...
        System.Console.WriteLine("Processed transfer to '{0}': {1}", stateMachine.Current.Value, result1.Message);

        var result2 = await stateMachine.Apply("Updating");
        System.Console.WriteLine("Processed transfer to '{0}': {1}", stateMachine.Current.Value, result2.Message);

        var result3 = await stateMachine.Apply("Finished");
        System.Console.WriteLine("Processed transfer to '{0}': {1}", stateMachine.Current.Value, result3.Message);
    }
    ...
    private static async Task DirectFailure(IServiceProvider provider)
    {
        ...
        System.Console.WriteLine("Processed transfer to '{0}': {1}", stateMachine.Current.Value, result.Message);
    }
